...
if __name__ == "__main__":
    ...
    for i, airport_names in enumerate(airport_names):
        ...
        google_ping_calc = google_pings[i] - tcp_pings[i] if google_pings[i] != 9999 and google_pings[i] - tcp_pings[i] > 0 else 9999
        results.append({"initiator": hostname,"data_type":"remote_test", "name": airport_names, "tcp_ping": tcp_pings[i], "google_ping": google_ping_calc, "public_ip": public_ips[i], "airport_config_b64": airport_config_b64 ,"created_at": ts})
